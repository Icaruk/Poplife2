...
if((typeOf _curTarget ) in listaCasas) exitWith {
		closeDialog 0;
	};
...
_Btn3 buttonSetAction "[life_pInact_curTarget] spawn ica_fnc_cacheo; closeDialog 0;";
...
_Btn9 ctrlSetText "Incautar dinero";
_Btn9 buttonSetAction "[[player],""life_fnc_robPerson"",life_pInact_curTarget,false] call life_fnc_MP;";

//Check that you are near a place to jail them.
if(!((player distance (getMarkerPos "police_hq_1") < 30) OR (player distance (getMarkerPos "cop_spawn_2") < 30) OR (player distance (getMarkerPos "cop_spawn_3") < 30) OR (player distance (getMarkerPos "cop_spawn_4") < 30) OR (player distance (getMarkerPos "cop_spawn_5") < 30))) then
{
