...
if(typeName _type == typeName []) then
{
	for "_i" from 0 to (count _type)-1 do
	{
		switch((_type select _i)) do
		{
			...
			case 3: {hintSilent format["%1", _message]};
            case 4: {[_message] call life_fnc_AAN;};
            case 5: {[_message] call life_fnc_nukeScene;};
			case 6: {[_message,(player getVariable["realname",name player])] call life_fnc_pubAAN;};
		};
	};
}
	else
