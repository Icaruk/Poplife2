...
if(parseNumber(_amount) > pop_din) exitWith {hint "No dispones de esa cantidad!";ctrlShow[2001,true];};
if(isNull _unit) exitWith {ctrlShow[2001,true];};
if(isNil "_unit") exitWith {ctrlShow[2001,true]; hint "El jugador seleccionado esta fuera de su rango";};
hint format["Diste $%1 A %2",[(parseNumber(_amount))] call life_fnc_numberText,_unit getVariable["realname",name _unit]];
pop_din = pop_din - (parseNumber(_amount));
