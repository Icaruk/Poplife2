	...
	else
{
	...
	if(_price > pop_din) then
	{
		_priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Price: <t color='#ff0000'>$%1</t><br/>Te Falta: <t color='#8cff9b'>$%2</t></t>",[(_price)] call life_fnc_numberText,[(_price - pop_din)] call life_fnc_numberText];
	}
		else
	{
		_priceTag ctrlSetStructuredText parseText format ["<t size='0.8'>Precio: <t color='#8cff9b'>$%1</t></t>",[(_price)] call life_fnc_numberText];
	};
};