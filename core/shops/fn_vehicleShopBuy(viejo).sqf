...
if ((life_inv_cb == 0) && (pop_din < _basePrice)) exitWith {hint "no tiene suficiente dinero en efectivo y no tiene tarjeta de credito !"};
...
if ((_gangbank >= _prixcb) && (life_inv_cb > 0)) then {
		...
		if (_action) then {
				...
				[ [0, format ["El vehiculo %1 fue comprado por %3 por el Precio %2$ Con el dinero de la Mafia", getText(configFile >> "CfgVehicles" >> _className >> "displayName"), [_prixcb] call life_fnc_numberText, name player]], "life_fnc_broadcast", _owner, false] call life_fnc_MP;
				//[ [player, "buycarniggah"], "A3L_Fnc_NearestSound", false, false, false] call life_fnc_MP;
				...
				if (_action) then {
						...
						if (_basePrice > pop_din) exitWith {hint "no tiene suficiente dinero en efectivo !"};
						if (pop_din >= _basePrice) then {
