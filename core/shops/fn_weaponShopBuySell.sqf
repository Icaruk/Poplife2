...
if((["pop_tarjeta"] call ica_fnc_tengo == 0) && (pop_din < _price)) exitWith {hint "No tiene suficiente dinero en efectivo y no tiene tarjeta de credito !"};
...
{
if((["pop_tarjeta"] call ica_fnc_tengo == 0) && (pop_din < _price)) exitWith {hint "No tiene suficiente dinero en efectivo y no tiene una tarjeta de credito !"};
if((_gangbank >= _prixcb) && (["pop_tarjeta"] call ica_fnc_tengo > 0)) then {
        _action = [
            format["Dinero de la Mafia : <t color='#8cff9b'>%1$</t><br/>CUENTA BANCARIA : <t color='#8cff9b'>%5$</t><br/>Efectivo : <t color='#8cff9b'>%2$</t><br/><br/>Precio en Efectivo : <t color='#8cff9b'>%3$</t><br/>Precio por Trajeta : <t color='#8cff9b'>%4$</t>",
                ...
                [pop_din] call life_fnc_numberText,
                [_price] call life_fnc_numberText,
                [_prixcb] call life_fnc_numberText,
                [pop_atmdin] call life_fnc_numberText
            ],
    ...
    "Mi dinero"
        ...
        _owner = (grpPlayer getVariable "gang_owner");
            ...
            [[0,format["Un arma %1 fue comprado por %3 por %2$ Con el dinero de la mafia",_itemInfo select 1,[_prixcb] call life_fnc_numberText,name player]],"life_fnc_broadcast",_owner,false] call life_fnc_MP;
        ...
        if(_action) then {
                ...
                if(_prixcb > pop_atmdin) exitWith {hint "No tiene suficiente dinero en su cuenta !"};
                pop_atmdin = pop_atmdin - _prixcb;
                ...
                if(_price > pop_din) exitWith {hint "No tiene suficiente dinero en efectivo !"};
                pop_din = pop_din - _price;
        ...
        if((["pop_tarjeta"] call ica_fnc_tengo == 0)) then {
                if(_price > pop_din) exitWith {hint "no tiene suficiente dinero en efectivo y no dispone de una tarjeta de credito !"};
                pop_din = pop_din - _price;
                hint parseText format["Compro un %1 por <t color='#8cff9b'>%2$</t>",_itemInfo select 1,[_price] call life_fnc_numberText];
                [_item,true] spawn life_fnc_handleItem;
        } else {
