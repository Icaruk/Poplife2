...
*/
private["_vehicle","_impound","_vInfo","_vInfo","_plate","_uid","_query","_sql","_unit","_veh"];
...
if(_impound) then
{
	...
	{
		_query = format["SetActiveVehicleByPidPlate:%1:%2:%3",_uid,_plate, fuel _veh];
	...
	else
{
	...
	_query = format["SetActiveVehicleByPidPlate:%1:%2:%3",fuel _vehicle,_uid,_plate];
	waitUntil {!DB_Async_Active};
	_thread = [_query,1] call DB_fnc_asyncCall;
	//waitUntil {scriptDone _thread};
	if(!isNil "_vehicle" && {!isNull _vehicle}) then {
