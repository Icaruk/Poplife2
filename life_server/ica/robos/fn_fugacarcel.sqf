...
if (_param isEqualTo "puertacentral") exitWith {
	...
	[[10, "Aviso", [1,0,0], 1.5, "El detector de explosivos de la carcel ha detectado algo", [1,1,1], 1], "ica_fnc_anuncio", WEST, false, false] call life_fnc_MP;
	sleep 120;
...
if (_param isEqualTo "abropuertas") exitWith {
	...
	[[_position], "life_fnc_castEMP", false] call life_fnc_MP;
	[[10, "Aviso", [1,0,0], 1.5, "Las puertas de la cárcel están abiertas, los presos se están escapando", [1,1,1], 1], "ica_fnc_anuncio", WEST, false, false] call life_fnc_MP;
...
if (_param isEqualTo "ejecuto") exitWith {
	...
	[[10, "El dpto. penitenciario de Plata o Plomo informa", [1,0,0], 1.5, format["Se ha impartido justicia, %1 acaba de ser ejecutado en la silla eléctrica",_nombrevictima], [1,1,1], 1], "ica_fnc_anuncio", true, false, false] call life_fnc_MP;
};
