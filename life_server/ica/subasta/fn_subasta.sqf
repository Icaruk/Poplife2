...
if (_param == "pujo") exitWith {
	...
	_empiezaPuja = {
		if ((subastaActual  == "") AND ((((missionnamespace getVariable _cosa) select 0) == ""))) then { 	// si no hay ninguna subasta activa
			...
			[[_cosa], "ica_fnc_initSubasta", false, false, false] call life_fnc_MP;
		};
	};
	...
	if (pop_atmdin < (((missionnamespace getVariable _cosa) select 1) + _nPuja)) exitWith {hint "No tienes dinero para superar la puja máxima"};
