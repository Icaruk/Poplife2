...
if (_param isEqualTo "doy") exitWith {
	...
	hint format["Le has dado a %1 las llaves de %2",_quien getVariable["realname",name _quien], getText(configFile >> "CfgVehicles" >> (typeOf _llave) >> "displayName")];
	[[_llave,_quien,profileName], "TON_fnc_clientGetKey",_quien,false] call life_fnc_MP;
};
