		...
		while {true} do {
			if (_t <= 0) exitWith {
				if (stance player == "STAND") exitWith {
					...
					[[player,"AwopPercMstpSgthWrflDnon_start1"],"life_fnc_animSync",nil,false] call life_fnc_MP;
					sleep 1.5;
				...
				if (stance player == "CROUCH") exitWith {
					...
					[[player,"AwopPknlMstpSgthWrflDnon_start"],"life_fnc_animSync",nil,false] call life_fnc_MP;
					sleep 1.5;
				...
				if (stance player == "PRONE") exitWith {
					...
					[[player,"AwopPpneMstpSgthWrflDnon_start"],"life_fnc_animSync",nil,false] call life_fnc_MP;
					sleep 1.5;
