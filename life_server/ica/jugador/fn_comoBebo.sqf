...
if (_param isEqualTo "init") exitWith {
				...
				if ((_estoy != _ultimaPos) && (vehicle player == player)) then {
					...
					if (_andado == _max) then	{
						...
						if ((player distance (getMarkerPos "jail_marker")) > 120) then {
							profileNamespace setVariable ["fois", (profileNamespace getVariable "fois") - 3];
	...
	[] spawn { // Agua
		while {true} do {
			...
			if ((player distance (getMarkerPos "jail_marker")) > 120) then {
				profileNamespace setVariable ["fois", (profileNamespace getVariable "fois") - 0.90];
				call ica_fnc_actualizaHUD;
			};
	...
	[] spawn { // Comida
		while {true} do {
			...
			if ((player distance (getMarkerPos "jail_marker")) > 120) then {			
				profileNamespace setVariable ["hearb", (profileNamespace getVariable "hearb") - 0.8];
				call ica_fnc_actualizaHUD;
			};
