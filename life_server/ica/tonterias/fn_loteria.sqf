	...
	while {true} do {
		...
		while {true} do {
			...
			if ((loteria select 0 select 0) <= (60 * 15)) exitWith {
				...
				[[10, "Lotería", [0,0,1], 1.5, _str, [1,1,1], 1], "ica_fnc_anuncio", CIVILIAN, false, false] call life_fnc_MP;
			};
...
if (_param isEqualTo "compro") exitWith {
	...
	if (pop_din < 5000) exitWith {hint format ["Te faltan %1€, pero la esperanza es lo último que se pierde", 5000 - pop_din]; closeDialog 0; ocupado = nil};
