...
if (_param isEqualTo "entierro") exitWith {
	...
	if (!(_muerto)) exitwith {hint "No hay nadia a quien enterrar"};
	if (isNil "_muerto") exitwith {hint "No hay nadia a quien enterrar"};
	...
	[_tumba , 60 * 30] call ica_fnc_borrame;
	
	if ((floor random 100) <= 90) then {
		[(["pop_ojos_item", "pop_corazon_item", "pop_corazon_item"] call BIS_fnc_selectRandom)] call ica_fnc_item; // PKE PENDIENTE
	};
