...
while {true} do {
	...
	if (!isNil {_dolor}) then {
		if (_dolor < 20) exitWith {_victima setVariable ["dolor", false]; hint "Fin dolor"; _fuera = true;};
	...
	if (!isNil {_dolor}) then {
		if ((random 100) < ((100 - _dolor) * 0.1)) then {
			if ((animationstate _victima) == "deadstate") then { // si no estoy inconsciente, no me levanto porque ya estoy de pie
