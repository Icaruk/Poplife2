...
if ((animationState player) == "deadstate") exitWith {};
...
if (_param isEqualTo "menu") exitWith {
	...
	if (vehicle player != player) then {
			...
			_cidx = switch (_tVeh) do {
					...
					case "ec135_ems_F": {[0, 1]};
					default {[5, 5]};
			};
		   
			if (vehicle player getCargoIndex player != (_cidx select 0)) exitWith {
					hint "Sólo puedes curar en una ambulancia y estando en un asiento próximo al enfermo";
					closedialog 0;
			};
...
if (_param isEqualTo "diag") then {
	...
	[_quien] spawn {
		...
		_txt ctrlSetStructuredText parseText format [" <t size='1.13' align='center'>%1</t> ", name quien];
