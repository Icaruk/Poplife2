...
if (_param == "rellena") exitWith {
	...
	_habilidades = switch (_rama) do {
		...
		case "for": {["Maratón", "Parkour", "Metabolismo efectivo", "Golpe de suerte", "Puñetazo poderoso", "Mula de carga"]};
		...
		case "inf": {["Explosivos", "Ojos en las paredes", "Hacker", "Free-WiFi", "PEM", "Ojos de gato"]};
	...
	_dataHab = switch (_rama) do {
		...
		case "for": {["for1", "for2", "for3", "for4", "for5", "for6"]};
		...
		case "inf": {["inf1", "inf2", "inf3", "inf4", "inf5", "inf6"]};
...
if (_param == "informa") exitWith {
	...
	_txt = switch (_habilidad) do {
		...
		case "med7": {"Ahora diagnosticas en un segundo y puedes usar el kit forense. ¡Puedes dar malas noticias más rápido!"};
		...
		case "for5": {"Puedes dar un puñetazo. En la cara."};
		...
		case "inf5": {"Puedes construir una granada PEM."};
		case "inf6": {"Puedes construir visión nocturna."};
		...
		case "maf8": {"Puedes comprar rifles de asalto. Espero que sepas lo que estás haciendo."};
	};	
...
if (_param isEqualTo "aprende") exitWith {
	...
	if (pop_din < _din) exitWith {hint format ["Por cada SP que gastes tienes que pagar 15.000€ \nAhora necesitas %1€", _din]};
