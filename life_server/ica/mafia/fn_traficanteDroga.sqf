...
if (_param isEqualTo "menu") exitWith {
	...
	if (_modo) then {
		...
		if (_tio == 1) then {
			...
			_pre1 ctrlSetText format ["%1 €", 1750*_mult];
			_pre2 ctrlSetText format ["%1 €", 2100*_mult];
			_pre3 ctrlSetText format ["%1 €", 1750*_mult];
			_pre4 ctrlSetText format ["%1 €", 1900*_mult];
		...
		if (_tio == 1) then {
			...
			_pre1 ctrlSetText format ["%1 €",round (500*(-0.0000005*traf1Mar*5*traf1Mar*5-0.00025*traf1Mar*5+1)*_mult)];
			_pre2 ctrlSetText format ["%1 €",round (950*(-0.0000005*traf1Her*5*traf1Her*5-0.00025*traf1Her*5+1)*_mult)];
			_pre3 ctrlSetText format ["%1 €",round (750*(-0.0000005*traf1Coc*5*traf1Coc*5-0.00025*traf1Coc*5+1)*_mult)];
			_pre4 ctrlSetText format ["%1 €",round (800*(-0.0000005*traf1Met*5*traf1Met*5-0.00025*traf1Met*5+1)*_mult)];
			_bar1 progressSetPosition (traf1Mar / _maxTraf);
			_bar2 progressSetPosition (traf1Her / _maxTraf);
			_bar3 progressSetPosition (traf1Coc / _maxTraf);
			_bar4 progressSetPosition (traf1Met / _maxTraf);
			...
			_pre1 ctrlSetText format ["%1 €",round (500*(-0.0000005*traf2Mar*5*traf2Mar*5-0.00025*traf2Mar*5+1)*_mult)];
			_pre2 ctrlSetText format ["%1 €",round (950*(-0.0000005*traf2Her*5*traf2Her*5-0.00025*traf2Her*5+1)*_mult)];
			_pre3 ctrlSetText format ["%1 €",round (750*(-0.0000005*traf2Coc*5*traf2Coc*5-0.00025*traf2Coc*5+1)*_mult)];
			_pre4 ctrlSetText format ["%1 €",round (800*(-0.0000005*traf2Met*5*traf2Met*5-0.00025*traf2Met*5+1)*_mult)];
			_bar1 progressSetPosition (traf2Mar / _maxTraf);
			_bar2 progressSetPosition (traf2Her / _maxTraf);
			_bar3 progressSetPosition (traf2Coc / _maxTraf);
			_bar4 progressSetPosition (traf2Met / _maxTraf);
...
if (_param isEqualTo "compro") exitWith {
	...
	if !(pop_din >= _pre) exitWith {hint "No tienes dinero para pagarme."};
...
// ["mochila_grande", 1] call ica_fnc_item; ["pop_cannabis_b", 5] call ica_fnc_item; pop_din = 1000000;