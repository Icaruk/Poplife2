	...
	*/
	...
	while {_idx < (count _arrayTienda)} do {
		...
		if ((typeName (_arrayTienda select _idx)) == "STRING") then { //
			...
			if (_modo) then {_precio = _precio * desc}; // soy donador y quiero mi descuento al comprar
