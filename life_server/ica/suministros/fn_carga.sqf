...
if (_pago > pop_din) exitWith {hint format ["Necesitas %1€ más para pagar los impuestos", _pago - pop_din]; };
...
_caja setdir 270;
// [[_caja, ["<t color='#DF0101'>Eliminar mágicamente</t>",'deleteVehicle (_this select 0)', "", 1,false,true,"","side player != CIVILIAN"]], "ica_fnc_meteAccion", true, true] call life_fnc_MP;
[[_caja, ["<t color='#DF0101'>Eliminar mágicamente</t>",'if (side player != CIVILIAN) then {deleteVehicle (_this select 0)}', "", 1,false,true,"",""]], "ica_fnc_meteAccion", true, true] call life_fnc_MP;
