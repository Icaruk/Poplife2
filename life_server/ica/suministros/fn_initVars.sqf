...
0 spawn {
	while {true} do {
			{
				missionNamespace setVariable [_x, (missionNamespace getVariable _x) - 12];
			...
			{
				missionNamespace setVariable [_x, (missionNamespace getVariable _x) - 10];
				if ((missionNamespace getVariable _x) <= 0) then {missionNamespace setVariable [_x, 0]};
			} forEach ["minAre", "minCar", "minCob", "minHie", "minOro", "minPla"];
			
			{
				missionNamespace setVariable [_x, (missionNamespace getVariable _x) - 5];
				if ((missionNamespace getVariable _x) <= 0) then {missionNamespace setVariable [_x, 0]};
